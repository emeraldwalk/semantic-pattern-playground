...
/***/ function(module, exports, __webpack_require__) {
	...
	var angular = __webpack_require__(3);
	var patterns_manifest_1 = __webpack_require__(9);
	exports.appModule = angular.module('patternSandbox', []);
	/**
	 ...
	 */
	function getConfigs(ext) {
	    return Object
	        .keys(patterns_manifest_1.patternManifest.map)
	        .filter(function (key) { return patterns_manifest_1.patternManifest.map[key].ext.indexOf(ext) > -1; })
	        .map(function (key) { return ({
	        name: patterns_manifest_1.patternManifest.map[key].name,
	        url: "" + key + ext,
	        hasData: patterns_manifest_1.patternManifest.map[key].ext.indexOf('.json') > -1
	    }); });
	}
	getConfigs('.html').forEach(function (config) {
	    exports.appModule.component(dashToCamelCase(config.name), {
	        templateUrl: function ($element) {
	            $element.addClass("c-" + config.name);
	            return config.url;
	        },
	        controller: (_a = (function () {
	                function class_1(_$http, _$scope) {
	                    this._$http = _$http;
	                    ...
	                    if (config.hasData) {
	                        this._$http.get(config.url.replace(/\.html$/, '.json'))
	                            ...
	                            for (var key in content.data) {
	                                _this._$scope[key] = content.data[key];
	                            }
	                        });
	                    }
	                };
